...
def pairwise_distance_matrix(shape_dict, method="similitude"):
    ...
    n = len(names)
    descriptors = {name: extract_descriptor(img, method=method)for name, img in shape_dict.items()}
    matrix = np.zeros((n, n))
...
# Classifier
# For every pair of shapes, predict match/no-match and return results; returns a list of dicts.
...
# Experiment runner
# Generic helper: apply transform at each param value, record distance.
...
def run_rotation(binary, shape_name, angles=None, method="similitude"):
    ...
    return _run(binary, lambda b, a: rotate_image(b, a), angles, "rotation", shape_name, method)

def run_scaling(binary, shape_name, scales=None, method="similitude"):
    if scales is None:
        scales = [0.4, 0.6, 0.75, 1.0, 1.25, 1.5, 1.75]

    return _run(binary, lambda b, s: scale_image(b, s), scales, "scaling", shape_name, method)
...
def run_gaussian_noise(binary, shape_name, sigmas=None, method="similitude"):
    ...
    return _run(binary, lambda b, s: add_gaussian_noise(b, sigma=s), sigmas, "gaussian_noise", shape_name, method)

def run_salt_pepper(binary, shape_name, amounts=None, method="similitude"):
    if amounts is None:
        amounts = [0.01, 0.03, 0.05, 0.08, 0.10, 0.15, 0.20]

    return _run(binary, lambda b, a: add_salt_pepper_noise(b, amount=a), amounts, "salt_pepper", shape_name, method)

def run_erosion(binary, shape_name, radii=None, method="similitude"):
    if radii is None:
        radii = [1, 2, 3, 5, 7, 10]

    return _run(binary, lambda b, r: erode_boundary(b, radius=r),radii, "erosion", shape_name, method)

def run_dilation(binary, shape_name, radii=None, method="similitude"):
    if radii is None:
        radii = [1, 2, 3, 5, 7, 10]

    return _run(binary, lambda b, r: dilate_boundary(b, radius=r), radii, "dilation", shape_name, method)
...
def run_blur(binary, shape_name, sigmas=None, method="similitude"):
    ...
    return _run(binary, lambda b, s: blur_and_threshold(b, sigma=s), sigmas, "blur", shape_name, method)

# Accuracy analysis
# Build same-class distances (original vs transformed variants) and different-class distances (cross-shape pairs).
def collect_same_diff_distances(shape_dict, method="similitude"):
    ...
    diff_distances = [dist_matrix[i, j] for i in range(n) for j in range(n) if i != j]
...
# Compare descriptor stability with and without full preprocessing; returns the intermediate images and the descriptor distance between the raw-binary version and the fully preprocessed version.
...
# Print overall stats table and per-distortion accuracy table.
...
def plot_experiment(x_vals, y_dict, baseline, xlabel, title, save_path=None, x_labels=None):
    ...
    for i, (label, y) in enumerate(y_dict.items()):
        ax.plot(x_vals, y, marker=markers[i], color=colors[i], linewidth=2, label=f"{label} (same-shape)")
        
    ax.axhline(baseline, color='black', linestyle='--', linewidth=1.5, label=f"Cross-shape baseline ({baseline:.4f})")
...
def plot_combined_noise(noise_levels, gn_dict, sp_dict, gn_baseline, sp_baseline, save_path=None):
    ...
    all_lines = (
        [(label, y, colors[i], markers[i], styles[i], "Gaussian") for i, (label, y) in enumerate(gn_dict.items())] +
        [(label, y, colors[2+i], markers[2+i], styles[2+i], "S&P")for i, (label, y) in enumerate(sp_dict.items())]
    )

    for label, y, color, marker, style, noise_type in all_lines:
        ax.plot(noise_levels, y, marker=marker, color=color, linestyle=style, linewidth=2, label=f"{label} [{noise_type}]")

    # Draw both baselines
    ax.axhline(gn_baseline, color='steelblue', linestyle=':', linewidth=1.5, label=f"Gaussian cross-shape baseline ({gn_baseline:.4f})")
    ax.axhline(sp_baseline, color='tomato', linestyle=':', linewidth=1.5, label=f"S&P cross-shape baseline ({sp_baseline:.4f})")

    ax.set_xlabel("Noise Level", fontsize=12)
    ax.set_ylabel("Euclidean Distance", fontsize=12)
    ax.set_title("Phase 3 – Distance vs Noise Level (Gaussian vs Salt-and-Pepper)", fontsize=13, fontweight='bold')
    ax.legend(fontsize=9)
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
...
# Imperfect segmentation graph; shows erosion (under-segmentation) vs dilation (over-segmentation for two shapes on one plot.
def plot_segmentation_error(radii, erosion_dict, dilation_dict, baseline, save_path=None):
    colors  = ['steelblue', 'royalblue', 'tomato', 'firebrick']
    markers = ['o', '^', 's', 'D']
    fig, ax = plt.subplots(figsize=(10, 6))

    for i, (label, y) in enumerate(erosion_dict.items()):
        ax.plot(radii, y, marker=markers[i], color=colors[i], linestyle='-', linewidth=2, label=f"{label} [erosion / under-seg]")

    for i, (label, y) in enumerate(dilation_dict.items()):
        ax.plot(radii, y, marker=markers[2+i], color=colors[2+i], linestyle='--', linewidth=2, label=f"{label} [dilation / over-seg]")

    ax.axhline(baseline, color='black', linestyle=':', linewidth=1.5, label=f"Cross-shape baseline ({baseline:.4f})")
    ax.set_xlabel("Morphological Radius (px)", fontsize=12)
    ax.set_ylabel("Euclidean Distance", fontsize=12)
    ax.set_title("Phase 3 – Imperfect Segmentation: Erosion vs Dilation", fontsize=13, fontweight='bold')
    ax.legend(fontsize=9)
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
...
def plot_histogram(same_distances, diff_distances, title="Distance Histogram", threshold=None, save_path=None):
...
def plot_distance_matrix(names, matrix, title="Pairwise Distance Matrix", save_path=None):
    ...
    for i in range(n):
        for j in range(n):
            ax.text(j, i, f"{matrix[i, j]:.3f}", ha='center', va='center', color='white' if matrix[i, j] > mid else 'black', fontsize=7)
    ax.set_title(title, fontsize=13, fontweight='bold')
    plt.tight_layout()
...
def plot_match_vs_mismatch(img_a1, img_b1, dist1, label_a1, label_b1, img_a2, img_b2, dist2, label_a2, label_b2, save_path=None):
    ...
    fig.text(0.5, 0.96, f"MATCH — distance: {dist1:.4f}", ha='center', fontsize=11, color='green', fontweight='bold')
    fig.text(0.5, 0.50, f"MISMATCH — distance: {dist2:.4f}", ha='center', fontsize=11, color='red', fontweight='bold')
...
def print_failure_report(shape_dict, method="similitude"):
    ...
    worst = sorted(per_dist, key=lambda row: row["same_mean"], reverse=True)[:2]
