...
# real silhouette loader
SILHOUETTE_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "silhouettes")

# load any image files found in the silhouettes folder and returns a dict w/ name and img
def load_silhouettes():
    ...
    for fname in sorted(os.listdir(SILHOUETTE_DIR)):
        ...
        try:
            ...
            # handle RGBA, palette, or grayscale images 
            if raw.ndim == 2:
...
def _cross_shape_distance(shape_a, shape_b):
    return euclidean_distance(extract_descriptor(shape_a, method=METHOD), extract_descriptor(shape_b, method=METHOD))
...
def phase2b():
    ...
    # use a noisy star to make preprocessing effects easier to see
    star = SHAPES["Star"]
...
def phase3():
    ...
    # rotation, asymmetric vs symmetric
    ...
    plot_experiment(angles, {"Triangle (asymmetric)": rot_tri, "Rectangle (symmetric)": rot_rect}, _cross_shape_distance(triangle, rectangle),
        "Rotation Angle (degrees)", "Phase 3 – Distance vs Rotation Angle", save_path=os.path.join(OUTPUT_DIR, "phase3_rotation.png"))
    print("  Saved: phase3_rotation.png")

    # scaling, simple vs complex boundary
    ...
    plot_experiment(scales, {"Circle (simple)": sc_circ, "Star (complex)": sc_star}, _cross_shape_distance(circle, star),
        "Scale Factor", "Phase 3 – Distance vs Scale Factor", save_path=os.path.join(OUTPUT_DIR, "phase3_scaling.png"))
    print("  Saved: phase3_scaling.png")

    # translation, circle vs triangle
    ...
    plot_experiment(list(range(len(shifts))), {"Circle": tr_circ, "Triangle": tr_tri}, _cross_shape_distance(circle, triangle),
        "Translation (dy, dx)", "Phase 3 – Distance vs Translation", save_path=os.path.join(OUTPUT_DIR, "phase3_translation.png"), x_labels=shift_labels)
    print("  Saved: phase3_translation.png")

    # Gaussian noise, sharp corners vs thin spikes
    ...
    plot_experiment(sigmas, {"Triangle (sharp corners)": gn_tri, "Star (thin spikes)": gn_star}, _cross_shape_distance(triangle, star), "Gaussian Noise Sigma", "Phase 3 – Distance vs Gaussian Noise", save_path=os.path.join(OUTPUT_DIR, "phase3_gaussian.png"))
    print("  Saved: phase3_gaussian.png")

    # salt & pepper, clean edges vs complex boundary
    ...
    plot_experiment(amounts, {"Rectangle (clean edges)": sp_rect, "Star (complex boundary)": sp_star}, _cross_shape_distance(rectangle, star),
        "Salt & Pepper Amount", "Phase 3 – Distance vs Salt-and-Pepper Noise", save_path=os.path.join(OUTPUT_DIR, "phase3_salt_pepper.png"))
    ...
    # boundary erosion, circle vs star
    ...
    plot_experiment(radii, {"Circle (smooth boundary)": er_circ, "Star (thin spikes)": er_star}, _cross_shape_distance(circle, star), "Erosion Radius (px)", "Phase 3 – Distance vs Boundary Erosion", save_path=os.path.join(OUTPUT_DIR, "phase3_boundary_damage.png"))
    ...
    # imperfect segmentation, erosion  vs dilation, testing both triangle and rectangle
    radii = [1, 2, 3, 5, 7, 10]
...
def phase4():
    ...
    plot_distance_matrix(names, matrix, title="Phase 4 – Pairwise Descriptor Distance Matrix", save_path=os.path.join(OUTPUT_DIR, "phase4_distance_matrix.png"))
    ...
    if SILHOUETTES:
        ...
        plot_distance_matrix(all_names, all_matrix, title="Phase 4 – Synthetic + Silhouette Pairwise Distance Matrix", save_path=os.path.join(OUTPUT_DIR, "phase4_silhouette_distance_matrix.png"))
    ...
    test_shapes = {
        "Circle": SHAPES["Circle"],
        "Circle Rot45": largest_component(rotate_image(SHAPES["Circle"], 45)),
        "Triangle": SHAPES["Triangle"],
        "Triangle Rot30": largest_component(rotate_image(SHAPES["Triangle"], 30)),
        "Ellipse": SHAPES["Ellipse"],
    }

    pair_results = classify_pairs(test_shapes, stats["threshold"], method=METHOD)

    print("\n  Pair Classification Results:")
    print(f"  {'Shape A':<18} {'Shape B':<18} {'Distance':>10} {'Prediction':>12}")
    print("  " + "-" * 64)

    for row in pair_results:
        prediction = "Match" if row["predicted_match"] else "No Match"
        print(f"  {row['shape_a']:<18} {row['shape_b']:<18} "
            f"{row['distance']:>10.4f} {prediction:>12}")
        
    plot_histogram(same_dists, diff_dists, title="Phase 4 – Same-Shape vs Different-Shape Distances", threshold=stats["threshold"], save_path=os.path.join(OUTPUT_DIR, "phase4_histogram.png"))
    print("  Saved: phase4_histogram.png")

    circle = SHAPES["Circle"]
    ellipse_s = SHAPES["Ellipse"]
    matched = largest_component(rotate_image(circle, 45))
    d_match = euclidean_distance(extract_descriptor(circle, method=METHOD), extract_descriptor(matched, method=METHOD))
    d_mismatch = euclidean_distance(extract_descriptor(circle, method=METHOD), extract_descriptor(ellipse_s, method=METHOD))
    plot_match_vs_mismatch(circle, matched,   d_match, "Circle (original)", "Circle (rotated 45°)", circle, ellipse_s, d_mismatch, "Circle", "Ellipse", save_path=os.path.join(OUTPUT_DIR, "phase4_match_vs_mismatch.png"))
